...
std::string                             BencodeParser::GetBencodeKeyContent(std::string const& json_string, std::string const& key) // turn string JSON to map <string, string>
{
    ...
    if (key == "length" || key == "piece length")
        found = found + tempo_key.length();
    else
...
void                                    BencodeParser::UseBencodeParser() {
    ...
    this->SetTorrentLength(this->GetBencodeKeyContent(t, "length"));
    this->SetPieceLength(this->GetBencodeKeyContent(t, "piece length"));
    this->SetTorrentName(this->GetBencodeKeyContent(t, "name"));
    ...
    std::cout << this->GetBencodeKeyContent(t, "piece length") << std::endl; // left ?
